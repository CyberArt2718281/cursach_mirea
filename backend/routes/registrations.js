...
router.post('/', async (req, res) => {
	try {
		console.log(
			'📝 POST /api/registrations - тело запроса:',
		...
		if (existingRegistration) {
			return res
				.status(400)
				.json({ error: 'Вы уже зарегистрированы на это событие' })
		}
...
router.put('/:id', auth, async (req, res) => {
	try {
		...
		const updatedRegistration = await Registration.findById(
			registration._id
		).populate('event')
...
router.patch('/:id/attend', auth, async (req, res) => {
	try {
		...
		if (registration.status !== 'подтверждена') {
			return res
				.status(400)
				.json({ error: 'Можно отметить только подтвержденные регистрации' })
		}
