...
router.get('/', async (req, res) => {
  try {
    const { category, status, search, sort = '-date', page = 1, limit = 10 } = req.query;
...
router.delete('/:id', auth, async (req, res) => {
  try {
    ...
    const activeRegistrations = registrations.filter(r => r.status === 'подтверждена');
    ...
    if (registrations.length > 0) {
      ...
      console.log(`🗑️ Удалено ${registrations.length} регистраций для события ${event.title}`);
    }
...
router.get('/:id/stats', auth, async (req, res) => {
  try {
    ...
    const stats = {
      ...
      confirmedRegistrations: registrations.filter(r => r.status === 'подтверждена').length,
      pendingRegistrations: registrations.filter(r => r.status === 'ожидание').length,
      cancelledRegistrations: registrations.filter(r => r.status === 'отменена').length,
      attendedCount: registrations.filter(r => r.attended).length,
      availableSeats: event.availableSeats,
      capacity: event.capacity,
      occupancyRate: ((registrations.filter(r => r.status === 'подтверждена').length / event.capacity) * 100).toFixed(2)
    };
